<!-- BAS DE PAGE -->
<!DOCTYPE html>
<html>

  <head>
    <meta charset="utf-8">
    <!--Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Montserrat:400,900|Ubuntu" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Noto+Sans&display=swap" rel="stylesheet">
    <!--CCS Stylsheet-->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    <link rel="stylesheet" href="./public/css/footer.css">
    <link rel="stylesheet" href="./public/css/mdb.min.css">
  	
    <!--Font Awesome-->
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css" integrity="sha384-UHRtZLI+pbxtHCWp1t77Bi1L4ZtiqrqD80Kn4Z8NTSRyMA2Fd33n5dQ8lWUE00s/" crossorigin="anonymous">

    <script src="./public/js/js.js"></script>
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
  </head>



    <!-- Footer -->
  <footer class="page-footer font-small  pt-4">

  <!-- Footer Links -->
  <div class="container text-center text-md-left">

    <!-- Grid row -->
    <div class="row">

      <!-- Grid column -->
      <div class="col-md-4 mx-auto">

        <!-- Content -->
        <h2 class="font-weight-bold">Qcm Live</h2>
        <p class="text-white">Dans le but de rendre les cours d'amphithêatre plus vivant et améliorer votre aprentissage de nouvelles notions, QCM Live vous permettra de tester vos connaissances en temps réel! </p> 

      </div>
      <!-- Grid column -->

      <hr class="clearfix w-100 d-md-none">

      <!-- Grid column -->
      <div class="col-md-2 mx-auto">

        <!-- Links -->
        <br>
        <h5 class="font-weight-bold text-uppercase text-white mt-3 mb-4" >Navigation</h5>

        <ul class="list-unstyled">
          <li>
            <a class="text-white text-uppercase"href="">Se connecter</a>
          </li>
  		
          <li>
            <a class="text-white text-uppercase"href="">S'inscrire</a>
          </li>
  		
        <li>
            <a class="text-white text-uppercase"href="">Utilisation</a>
         </li>
  	   
  	   <li>
            <a class="text-white text-uppercase"href="">Les Développeurs</a>
          </li>
        </ul>

      </div>
      <!-- Grid column -->

      <hr class="clearfix w-100 d-md-none">

      <!-- Grid column -->
      <div class="col-md-2 mx-auto">

        <!-- Links -->
        <br>
        <h5 class="font-weight-bold text-uppercase text-white mt-3 mb-4">Ressources</h5>

        <ul class="list-unstyled">
          <li>
            <a class="text-white text-uppercase"href="">FAQ</a>
          </li>
          <li>
            <a class="text-white text-uppercase"href="">Blog</a>
          </li>
          <li>
           <a class="text-white text-uppercase"href="">Gestion des cookies</a>
          </li>
          <li>
           <a class="text-white text-uppercase"href="">Mention légales</a>
          </li>
        </ul>
        
      </div>
      <!-- Grid column -->

      <hr class="clearfix w-100 d-md-none">

      <!-- Grid column -->
      <div class="col-md-3 mx-auto">

        <!-- Links -->
        <br>
        <h5 class="font-weight-bold text-uppercase text-white mt-3 mb-4">Contact</h5>

        <ul class="list-unstyled list-inline ">
    <li class="list-inline-item">
      <a class="btn-floating btn-fb mx-1">
          <a href="https://fr-fr.facebook.com/iutparis/"> <i class="fab fa-facebook-square fa-2x"></i></a>
      </a>
    </li>
    <li class="list-inline-item">
      <a class="btn-floating btn-tw mx-1">
          <a href="https://twitter.com/iutparis?lang=fr"> <i class="fab fa-twitter fa-2x"></i></a>
      </a>
    </li>
    <li class="list-inline-item">
      <a class="btn-floating btn-gplus mx-1">
              <a href="https://www.instagram.com/uparisdescartes/?hl=fr"><i class="fab fa-instagram fa-2x"></i></a>
      </a>
    </li>
    <li class="list-inline-item">
      <a class="btn-floating btn-gplus mx-1"href="https://www.youtube.com/channel/UCRMeMH1kO0QeuEE_DEMDaiA"><i class="fab fa-youtube fa-2x"></i></a>
        
      </a>
    </li>
    
  </ul> 

      </div>
      <!-- Grid column -->

    </div>
    <!-- Grid row -->

  </div>
  <!-- Footer Links -->



  <!-- Social buttons -->

  <!-- Copyright -->
  <div class="footer-copyright text-center blue-gradient py-3">© 2019 Copyright:
    <a href="">QCM Live</a>
  </div>
  <!-- Copyright -->

  </footer>
  </body>
</html>
